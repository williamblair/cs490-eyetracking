...
#First attempt at pupil tracking, it's crazy
...
#Start circle finding
        ...
        circles = cv2.HoughCircles(focusEye, cv2.HOUGH_GRADIENT, 1, 20, param1=90, param2=20, minRadius=0,
                                   maxRadius=20)
        #print(eyeRect[1]) #Y
        #print(eyeRect[0]) #X
        cameraFrameY = eyeRect[1]+y
        cameraFrameX = eyeRect[0]+x
        showEye = frame[cameraFrameY:cameraFrameY+50 , cameraFrameX:cameraFrameX+50]

    if hasCircles(circles):
        print(circles)
        cv2.circle(frame, (int(circles[0][0][0])+x+eyeRect[0],int(circles[0][0][1])+y+eyeRect[1]), circles[0][0][2], (0, 255, 0), 2)
