# following anp.double with tango with code guide
import numpy as np
import cv2
import sys
import win32api
        ...
        if not (eyes is None):
            ...
            if (not (eyes is None)) and not len(eyes) == 0:
                ...
                if not (irises is None):
                    ...
                    #print eyeball
                    #print ew,eh,center[0],center[1]
                    #print ew/2, eh/2

                    if not (center is None):
                        diff = [0, 0]
                        absoluteCenter[0] = ew/2
                        absoluteCenter[1] = eh/2
                        thresh[0] = abs(center[0]-absoluteCenter[0])
                        thresh[1] = abs(center[1]-absoluteCenter[1])
                        #print 'thresh',thresh[0],thresh[1]

                        cv2.circle(frame,(absoluteCenter[0]+fx+ex,absoluteCenter[1]+fy+ey),5,(255,0,0),2)

                        if(center[0]>absoluteCenter[0] ):
                            mousePoint[0] += 30
                        if(center[1]>absoluteCenter[1] ):
                            mousePoint[1] += 30
                        if(center[0]<absoluteCenter[0] ):
                            mousePoint[0] -= 30
                        if(center[1]<absoluteCenter[1] ):
