...
#This was the first attempt at controlling the mouse with the pupil
#The method at which we scale the cursor is bad though
...
# holds the previous centerq
lastPoint = [0,0]

# holds the lqqocation of the mouse
        ...
        if not (eyes is None):
            ...
            if (not (eyes is None)) and not len(eyes) == 0:
                ...
                if not (irises is None):
                    ...
                    if(runNum == 5):
                        initCenter[0] = fx + ex + center[0]
                        initCenter[1] = fy + ey+center[1]

                    runNum = runNum+1

                    calcCenter[0] = center[0] + fx + ex
                    calcCenter[1] = center[1] + fy + ey

                    if(runNum > 5):
                    # calculate the new mouse position
                        if not (center is None):
                            diff = [0,0]
                            print(initCenter[0] , " " , initCenter[1] , " " , calcCenter[0] ," " , calcCenter[1])
                            diff[0] = (initCenter[0] - calcCenter[0]) * MOUSE_SCALE_X
                            diff[1] = (initCenter[1] - calcCenter[1]) * MOUSE_SCALE_Y
                            print(diff[0],'',diff[1])
                        #print 'Diff: ', diff
